...
def login_facial():
    ...
    cv2.imwrite(usuario_login + "LOG.jpg", frame)  # Guardamos la última captura del video como imagen y asignamos el nombre del usuario
    cap.release()  # Cerramos
    cv2.destroyAllWindows()
    ...
    im_archivos = os.listdir()  # Vamos a importar la lista de archivos con la librería os
    if usuario_login + ".jpg" in im_archivos:  # Comparamos los archivos con el que nos interesa
        ...
        if similitud >= 0.98:
            ...
            # Mostrar el mensaje de bienvenida en la interfaz de usuario (pantalla2)
            bienvenida_msg = f"Bienvenido {usuario_login}!"
            Label(pantalla2, text=bienvenida_msg, fg="blue", font=("Calibri", 14)).pack()  # Aquí agregamos el mensaje de bienvenida
